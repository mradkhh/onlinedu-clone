  ...
  return (
    <div className="cases__item">
      <div className="cases__item-info">
        <span className='cases__item-num'>{item.case_id}</span>
        <h6 className='cases__item-title'>{item.case_title}</h6>
      ...
      <div className="cases__item-score">{item.case_score}</div>
    </div>
  )
}

const TestCase = ({cases, explanation}) => {
  ...
  const addCase = () => {
    const newCasesList = testData.data.map(test => {
      if (test.id) {
        ...
        return {
          ...test,
          ...test.cases.push({case_title: caseTitleValue, id: uuidv4()})
        }
      } else {
        console.log("wrong")
        return {...test}
